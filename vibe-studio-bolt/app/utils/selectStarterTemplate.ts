...
const parseSelectedTemplate = (llmOutput: string | undefined): { template: string; title: string } | null => {
  try {
    if (!llmOutput) {
      return null;
    }
...
export const selectStarterTemplate = async (options: { message: string; model: string; provider: ProviderInfo }) => {
  ...
  const response = await fetch(apiUrl('/api/llmcall'), {
    method: 'POST',
...
const getGitHubRepoContent = async (repoName: string): Promise<{ name: string; path: string; content: string }[]> => {
  try {
    ...
    const response = await fetch(apiUrl(`/api/github-template?repo=${encodeURIComponent(repoName)}`));
