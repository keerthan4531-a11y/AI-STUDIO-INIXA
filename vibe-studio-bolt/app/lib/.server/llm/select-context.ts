...
---
  ...
  if (!updateContextBuffer) {
    logger.error('Invalid response from context selector. Falling back to empty file selection.');
    return {};
  }
  ...
  if (totalFiles == 0) {
    logger.warn(`Bolt failed to select files, returning empty file selection.`);
    return {};
  }
