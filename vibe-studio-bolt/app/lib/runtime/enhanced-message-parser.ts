...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  private _blockArtifactIds = new Map<string, string>();
  private _lastEnhancedInputs = new Map<string, string>();
  ...
  parse(messageId: string, input: string): string {
    ...
    if (!this._hasDetectedArtifacts(input)) {
      ...
      const lastEnhanced = this._lastEnhancedInputs.get(messageId) || "";

      if (enhancedInput !== input) {
        if (enhancedInput.startsWith(lastEnhanced)) {
          // Past was not mutated! Just parse normally.
          this.wasReset = false;
          this._lastEnhancedInputs.set(messageId, enhancedInput);
          output = super.parse(messageId, enhancedInput);
        } else {
          // Past WAS mutated! (e.g. ``` turned into <boltArtifact>)
  ...
  private _detectAndWrapCodeBlocks(messageId: string, input: string): string {
    ...
    enhanced = this._detectAndWrapShellCommands(messageId, enhanced);
    ...
    for (const pattern of patterns) {
      enhanced = enhanced.replace(pattern.regex, (match, ...args) => {
        ...
        let artifactId = this._blockArtifactIds.get(blockHash);
        if (!artifactId) {
           artifactId = `artifact-${messageId}-${this._artifactCounter++}`;
           this._blockArtifactIds.set(blockHash, artifactId);
        }
        ...
        if (this._isShellCommand(content, language)) {
          ...
          return this._wrapInShellAction(content, artifactId);
        }
...
${content}
  ...
  private _wrapInShellAction(content: string, artifactId: string): string {
...
${content.trim()}
  ...
  private _detectAndWrapShellCommands(messageId: string, input: string): string {
  ...
  reset() {
    super.reset();
    this._blockArtifactIds.clear();
    this._lastEnhancedInputs.clear();
