...
async function scrapeKeys(): Promise<ScrapedKeyEntry[]> {
  try {
    ...
    for (const line of lines) {
      ...
      else if (line.startsWith("### Claude Opus"))
        currentCategory = "claude-opus-4-7";
      else if (line.startsWith("### Gemini"))
        currentCategory = "gemini-2.5-flash";
      else if (line.startsWith("### DeepSeek"))
        currentCategory = "deepseek-chat";
      else if (line.startsWith("### Multi-Model"))
        currentCategory = "smart-chat";
      else if (line.startsWith("### Kimi")) currentCategory = "kimi-k2.5";
      else if (line.startsWith("### Image")) currentCategory = "text-embedding";

      const match =
        /\|\s*`(sk-[a-zA-Z0-9_-]+)`\s*\|\s*([a-zA-Z0-9._\/-]+)\s*\|/.exec(line);
      if (match) {
        entries.push({
          key: match[1],
          model: match[2],
...
export async function POST(req: Request) {
  ...
  const userApiKey = req.headers.get("x-g4f-api-key");
  ...
  try {
    ...
    if (adminStats.isIpBanned(ip)) {
      return NextResponse.json(
        { ok: false, error: "Forbidden: IP Banned" },
        { status: 403 },
      );
    }
    ...
    const hasValidServerSecret =
      SERVER_SECRET && authHeader && authHeader.includes(SERVER_SECRET);
    const isScrapedKeyRequest =
      authHeader && (authHeader.includes("sk-") || authHeader.includes("g4f_"));

    if (!isOriginAllowed && !hasValidServerSecret && !isScrapedKeyRequest) {
      console.warn(
        `[Security] Blocked unauthorized request from Origin: ${origin}`,
      );
      return NextResponse.json(
    ...
    if (
      authHeader &&
      !authHeader.includes("g4f_") &&
      (!SERVER_SECRET || !authHeader.includes(SERVER_SECRET))
    ) {
      ...
      if (isScrapedKeyRequest && model !== "g4f/gpt-4o") {
        const now = Date.now();
        if (
          now - lastScrapeTime > SCRAPE_INTERVAL ||
          cachedScrapedKeys.length === 0
        ) {
        ...
        if (model === "gpt-5.5")
          matchingKeys = cachedScrapedKeys.filter(
            (k: any) => k.category === "gpt-5.5",
          );
        else if (model === "claude-opus-4.7")
          matchingKeys = cachedScrapedKeys.filter(
            (k: any) => k.category === "claude-opus-4-7",
          );
      ...
      if (stream) {
        const bodyStream = new ReadableStream({
          start(controller) {
            (backendRes.body as any).on("data", (chunk: Buffer) =>
              controller.enqueue(chunk),
            );
            (backendRes.body as any).on("end", () => controller.close());
            (backendRes.body as any).on("error", (err: Error) =>
    ...
    // 2. Direct OverChat Server Handler (Avoids CORS & CF Worker 403) with REAL Model Fallbacks
        ...
        if (stream && !contentType.includes("application/json")) {
          const bodyStream = new ReadableStream({
            start(controller) {
              ...
              (res.body as any).on("end", () => controller.close());
              (res.body as any).on("error", (err: Error) =>
                controller.error(err),
              );
            },
            cancel() {
    ...
    // 3. Fallback Route
    throw new Error(
      `Invalid model prefix. Must start with g4f/ or use valid API key. Model: ${model}`,
    );
  } catch (error: any) {
