...
import { Search, MessageSquare, Image as ImageIcon, Video, ChevronDown, Plus, X, Download, Settings2, FileCode, Trash2, Info, Sparkles, Code2, Mic, MoreHorizontal } from 'lucide-react';
...
export type TabType = 'video' | 'image' | 'chat' | 'codex' | 'pdf' | 'profile' | 'about' | 'vibe' | 'api' | 'voice';
...
export function DesktopSidebar({ user, activeTab, onTabChange, onNewChat, onSelectSession, onDeleteSession, chatSessions = [], activeSessionId, onOpenModelSelector, chatKey, setChatKey }: SidebarProps) {
  if (activeTab === 'vibe') {
    return (
      <aside className="hidden lg:flex flex-col w-[60px] items-center bg-[#0b0c14]/50 backdrop-blur-3xl border-r border-white/[0.05] sticky top-0 z-[100] h-screen shrink-0 py-6 transition-all">
  ...
  return (
    <aside className="hidden lg:flex flex-col w-[260px] bg-[#0b0c14]/50 backdrop-blur-3xl border-r border-white/[0.05] sticky top-0 z-[100] h-screen shrink-0 transition-all">
      <div className="p-6 flex flex-col h-full">
        <div className="flex items-center gap-3 mb-10">
          <InixaLogo size={36} className="rounded-xl shadow-lg shadow-indigo-500/10" />
          <h1 className="text-[17px] font-black tracking-tight uppercase leading-none text-white">Inixa <span className="text-indigo-400 font-medium normal-case text-[13px]">Preview</span></h1>
        </div>
        ...
        <div className="space-y-1 mb-8">
          {[
            ...
            >
              ...
              {item.kbd && <span className="text-[10px] text-white/20 font-bold px-1.5 py-0.5 rounded border border-white/10 group-hover:border-white/20">{item.kbd}</span>}
            </button>
          ))}
        </div>
