...
import { ChevronUp, ChevronDown, Download, FileText, MonitorPlay, Sparkles, Image as ImageIcon, Quote, Maximize } from 'lucide-react';
...
export function SlideDeckViewer({ content, title }: SlideDeckViewerProps) {
  ...
  return (
    <div ref={containerRef} className="flex flex-col bg-[#020617] border border-white/10 rounded-3xl overflow-hidden shadow-2xl relative h-[600px] w-full">
      ...
      <div className="flex items-center justify-between p-4 bg-white/[0.02] border-b border-white/5 backdrop-blur-xl absolute top-0 w-full z-50">
        ...
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-xs font-bold text-white uppercase tracking-wider transition-all"
          >
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 disabled:opacity-50 px-4 py-2 rounded-xl text-xs font-bold text-white uppercase tracking-wider transition-all shadow-md shadow-cyan-500/20"
          >
      ...
      {createPortal(
        <div id="slide-deck-print-section-root" className="hidden print:block bg-[#020617] text-white">
          <style>{`
            @media print {
              html, body {
                margin: 0 !important;
              ...
              body > *:not(#slide-deck-print-section-root) {
                display: none !important;
              }
              #slide-deck-print-section-root {
          ...
          `}</style>
          {deck.cards.map((card, idx) => (
            <div key={idx} className="print-slide-page relative overflow-hidden border border-white/5 shadow-2xl">
              ...
              {card.layout === 'hero' && (
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-12">
                  ...
                  <h1 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-500 mb-6 drop-shadow-2xl relative z-10 leading-tight">
                    {card.title}
                  </h1>
                  {card.subtitle && <p className="text-xl text-cyan-100/70 font-medium relative z-10 max-w-3xl mx-auto">{card.subtitle}</p>}
                </div>
              )}
              ...
              {(card.layout === 'split_left' || card.layout === 'split_right') && (
                <div className={`absolute inset-0 flex ${card.layout === 'split_right' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className="flex-1 p-12 flex flex-col justify-center">
                    <h2 className="text-4xl font-bold text-white mb-6 leading-tight">{card.title}</h2>
                    {card.subtitle && <h3 className="text-xl text-cyan-400 mb-6 font-semibold">{card.subtitle}</h3>}
                    {card.content && <p className="text-base text-slate-300 leading-relaxed mb-8">{card.content}</p>}
              ...
              {(card.layout === 'grid' || (!['hero','split_left','split_right','quote'].includes(card.layout))) && (
                <div className="absolute inset-0 p-12 flex flex-col">
                  ...
                  <div className={`grid gap-6 flex-1 ${card.bullets && card.bullets.length > 3 ? 'grid-cols-2' : 'grid-cols-1 md:grid-cols-2'}`}>
                    {card.bullets?.map((b, i) => (
                      <div key={i} className="bg-white/[0.03] border border-white/[0.05] rounded-3xl p-6 flex flex-col gap-4 relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/10 blur-[50px]" />
                        <h4 className="text-cyan-400 font-bold text-base tracking-wide uppercase">Point {i+1}</h4>
                        <p className="text-lg text-white font-medium leading-relaxed">{b}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {card.layout === 'quote' && (
                <div className="absolute inset-0 p-12 flex flex-col items-center justify-center text-center bg-gradient-to-b from-[#1e1b4b] to-[#0f172a]">
                  <Quote className="w-20 h-20 text-cyan-500/30 mb-6" />
                  <h2 className="text-4xl font-serif italic text-white leading-tight max-w-4xl mx-auto">"{card.title}"</h2>
                  {(card.subtitle || card.content) && (
                    <div className="mt-8 flex items-center gap-4">
                      <div className="w-12 h-[1px] bg-cyan-500" />
                      <p className="text-lg font-bold text-cyan-400 uppercase tracking-widest">{card.subtitle || card.content}</p>
                      <div className="w-12 h-[1px] bg-cyan-500" />
                    </div>
                  )}
                </div>
